...
public class EmpDao {  
    ...
    public static int save(Emp e){  
        int status=0;  
        try{  
            Connection con=Connection_pool.getDataSource().getConnection();  
            PreparedStatement ps=con.prepareStatement(  
                         "insert into student_info(NAME,PASSWORD,EMAIL,COUNTRY) values (?,?,?,?)");  
    ...
    public static int update(Emp e){  
        ...
        try{  
        	Connection con=Connection_pool.getDataSource().getConnection();   
            PreparedStatement ps=con.prepareStatement(  
                         "update student_info set NAME=?,PASSWORD=?,EMAIL=?,COUNTRY=? where ID=?");  
            ps.setString(1,e.getName());  
            ps.setString(2,e.getPassword());  
            ps.setString(3,e.getEmail());  
            ps.setString(4,e.getCountry());  
